	...
	d.wrappedText = function(a, b, c, e, h, f, g, k, l) {
		...
		if (m) {
			...
			if (q.width > k) {
				...
				for (var r, p = 0; p < m.length; p += Math.ceil(m.length /
					q)) r = m[p], b = b.substr(0, r) + n + b.substr(r + 1);
				if (isNaN(r)) {
					if (0 === l)
	...
	d.formatValue = function(a, b, c, e, h, f, g, k) {
		if (b) {
			...
			for (l = 0; l < c.length; l++) {
				var m = c[l],
					n = b[m];
				void 0 !== n && (n = f ? d.addPrefix(n, k, g, e) : d.formatNumber(n, e), a = a.replace(new RegExp("\\[\\[" +
					h + "" + m + "\\]\\]", "g"), n))
			}
		}
		return a
		...
		parseDatesDraw: function() {
			...
			if (0 < this.gridCountR)
				for (a = k; a <= B; a++) {
					...
					m = l = p;
					if (!isNaN(u))
						for (h = 1; h < w; h += u) this.gridAlpha = this.minorGridAlpha, f = p + v * h, f = d.resetDateToMin(new Date(f), x, u, r).getTime(), f = new this.axisItemRenderer(this, (f - this.startTime) * this.stepWidth), this.pushAxisItem(f);
					this.gridAlpha = z
				}
		},
	...
	d.RecItem = d.Class({
		construct: function(a, b, c, e, h, f, g, k, l, m, n, q) {
			...
			if ("H" == Pa) {
				...
				y += t / 2;
				"right" == aa ? (G += qa + sa + ba, y += ja, U ? (f || (y -= t / 2 + 3), G = G - (B + 4) - na) : (G += B + 4 + w, y -= 2, G += na)) : U ? (G += B + 4 - ba, f || (y -= t / 2 + 3), g && (G += qa, y += ja), G += na) : (G += -B -
					w - 4 - 2 - ba, y -= 2, G -= na);
			...
			for (K = da; K <= pa; K++) {
				...
				if (Ba) {
					...
					switch (g) {
						...
						case "step":
							if (!isNaN(X)) {
								r ? (isNaN(x) || (v.push(x), w.push(E - ra)), w.push(E -
									ra), v.push(F), w.push(E + Fa), v.push(F), !Ka || isNaN(L) || isNaN(N) || (isNaN(B) || (R.push(B), S.push(L - ra)), R.push(N), S.push(L - ra), R.push(N), S.push(L + Fa))) : (isNaN(A) || (w.push(A), v.push(x), w.push(A), v.push(F - ra)), v.push(F - ra), w.push(E), v.push(F + Fa), w.push(E), !Ka || isNaN(L) || isNaN(N) || (isNaN(D) || (R.push(N - ra), S.push(D)), R.push(N - ra), S.push(L), R.push(N + Fa), S.push(L)));
								x = F;
								A = E;
								B = N;
								D = L;
								...
								if (1 >= this.periodSpan || 1 < this.periodSpan && F - x > ra + Fa) x = A = NaN;
								this.drawLineGraph(v, w, R, S);
								v = [];
								w = [];
								R = [];
		...
		drawLineGraph: function(a, b, c, e) {
			...
			if (1 < a.length) {
				...
				var u = h.negativeLineColor,
					...
					x = h.baseCoord;
				0 !== h.negativeBase && (x = h.valueAxis.getCoordinate(h.negativeBase));
					...
					if (c.mouseIsOver || B || D || this.forceShow)
						if (this.forceShow = !1, D) {
							...
							0 !== h && (k = {
								type: "zoomed",
								target: this
							}, k.chart = this.chart, g.parseDates && !g.equalSpacing ? (c + h > x && (h = x - c), B + h < w && (h = w - B), k.start = Math.round(B + h), k.end = Math.round(c + h), this.fire(k.type, k)) : I + h >= this.data.length || 0 > m + h || (k.start =
								m + h, k.end = I + h, this.fire(k.type, k)))
						} else {
			...
			if (!isNaN(a)) {
				...
				if (this.show) {
					...
					var k = A + u,
						...
						O = this.bulletSize,
						J;
					0 < w || 0 === z ? (0 < u && (a = d.rect(h, C, x, q, 0, t + 1, H, u, this.cornerRadius), d.isModern ?
						...
						r, w, q, this.fillAlpha, t, p, this.borderAlpha));
					this.bg = q;
					f.push(q);
					q.toFront();
		...
		updateChartScrollbar: function(a, b) {
			if (a) {
				...
				var c = this.marginTopReal,
					...
					g = this.dy,
					k = a.offset;
				"top" == a.position ? b ? (a.y = c, a.x = e - d - k) : (a.y = c - d + g - k, a.x = e +
					f) : b ? (a.y = c + g, a.x = e + this.plotAreaWidth + f + k) : (a.y = c + this.plotAreaHeight + k, a.x = this.marginLeftReal)
			}
		},
		showZB: function(a) {
			var b = this.zbSet;
		...
		updateValues: function() {
			...
			for (c = 0; c < a.length; c++) {
				...
				if (d) f.value ? h.text(f.value) : h.text("");
					...
					} else g = b.formatString(this.valueText, f); if (l = this.valueFunction) k && (f = k), g = l(f, g);
					h.text(g)
				}
			}
		},
